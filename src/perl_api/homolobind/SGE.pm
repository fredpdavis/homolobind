...
homolobind::SGE - perl module for SGE cluster interaction
...
sub _clust_qsub {
   ...
   my $cwd = getcwd() ;
   my $status = `ssh $hostname \"cd $cwd; qsub $in->{sgescript_fn}\"` ;
   chomp $status;
...
sub _clust_qstat {
   ...
   my $in = shift;
   my $hostname = $in->{hostname} ;
   system("ssh $hostname qstat -j $in->{job_id} > /tmp/qstat.$$.out 2>&1") ;
